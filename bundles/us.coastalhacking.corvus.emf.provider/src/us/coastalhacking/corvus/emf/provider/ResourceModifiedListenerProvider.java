...
//		+ "=" + EmfApi.ResourceSetListener.Properties.ResourceModifiedListener.ID)
public class ResourceModifiedListenerProvider implements ResourceSetListener {
	...
	public void resourceSetChanged(ResourceSetChangeEvent event) {
		...
		Optional.ofNullable(tx.getChangeDescription()).ifPresent(desc -> {
			...
			if (!uris.isEmpty()) {
				SaveResourcesJob job = new SaveResourcesJob(uris, event.getEditingDomain(), getOptions());
	...
	void addUris(Set<URI> uris, TransactionChangeDescription description) {
		...
		// In EMFT, description.getObjectChanges() returns objects attached to the
		// graph, either newly added or pre-existing,
		// that have changed. description.getObjectsToDetach returns, for whatever
		// reason, those objects recently added
